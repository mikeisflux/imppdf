    ...
    <Section label="// TIFF EXPORT" help="Uncompressed, interleaved RGB TIFF with the W1 (white) and V1 (varnish) spot channels after it — the file your RIP opens for a black box.">
      <div className="pe-row" style={{ gap: 8, alignItems: 'center' }}>
        ...
        <select className="pe-select" value={dpi} onChange={(e) => up({ tiffDpi: Number(e.target.value) })} style={{ width: 130 }}>
          <option value={300}>300 (recommended)</option><option value={360}>360</option><option value={600}>600 (fine detail)</option>
        </select>
        <span style={{ flex: 1 }} />
    ...
    case 'fierybooklet':
      return (
        <>
          ...
          <div className="pe-note" style={{ marginBottom: 12 }}>
            ⓘ Trims a fixed <b>1/8&quot; (0.125&quot;)</b> bleed off the spine edge of every page — the Fiery standard. Exports single pages in reader order (not imposed spreads); each page keeps full bleed on its other three edges, so the Fiery / DFE booklet maker folds a clean booklet.
          </div>
        </>
      );
    case 'fieryserial': {
...
function RaisedMetalPanel({ s, up, sourceBytes }: PanelProps) {
  ...
  }, [ready, mode, s.subjectOnly, s.matteTighten, s.edgeGain, s.highlightGain, s.highlightFrom, s.toneGain, s.floor, s.gamma]);
  return (
    <>
      <div className="pe-note" style={{ marginBottom: 12 }}>
