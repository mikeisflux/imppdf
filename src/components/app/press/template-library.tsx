...
function layoutOf(steps: WorkflowStep[]) {
  const impose = steps.find((st) => ['grid', 'cards', 'cutstack', 'perfectbound', 'trading', 'bookmark', 'flyer', 'business', 'postcard', 'rackcard', 'hangtag', 'label', 'namebadge', 'ticket', 'coupon', 'placecard', 'greeting', 'doorhanger', 'envelope', 'coaster', 'contact', 'compslip', 'trifold', 'zfold', 'gatefold', 'menu', 'poster', 'banner', 'rollbanner', 'featherflag', 'yardsign', 'boxcarton', 'presfolder', 'magazine', 'catalog', 'program', 'notebook', 'hymnal', 'booklet', 'comic', 'zine', 'gangsheet', 'stickers', 'datamerge', 'customimpose', 'nupbook'].includes(st.type));
  const s = impose?.s ?? {};
  const duplex = !!s.duplex;
  if (!impose) return { cols: 1, rows: 1, duplex, kind: 'single' as const, s };
  if (['booklet','comic','magazine','catalog','program','notebook','hymnal','nupbook'].includes(impose.type)) return { cols: 2, rows: 1, duplex: true, kind: 'booklet' as const, s };
