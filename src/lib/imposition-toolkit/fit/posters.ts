/* Posters — large format, ONE piece per sheet.
 *
 * Wide-format work is printed to the piece, not ganged on a press sheet: the
 * media IS the poster. So there is no grid to compute and NO CUT MARKS —
 ...
 *
 * Rotation is off: a poster has a fixed orientation. Turning it to "fit
...
export const POSTER_W_IN = 24;
export const POSTER_H_IN = 36;
export const POSTER_TOOL = 'poster';
...
export function fitPoster(
  spec: SheetSpec, pieceWIn = POSTER_W_IN, pieceHIn = POSTER_H_IN,
): SheetFit {
  const f = packBestOrientation({ ...POSTER_DEFAULTS, ...spec }, pieceWIn, pieceHIn, 'piece');
