/* Banners — large format, ONE piece per sheet.
 *
 * Wide-format work is printed to the piece, not ganged on a press sheet: the
 * media IS the banner. So there is no grid to compute and NO CUT MARKS —
 ...
 *
 * Rotation is off: a banner has a fixed orientation. Turning it to "fit
...
export const BANNER_W_IN = 24;
export const BANNER_H_IN = 72;
export const BANNER_TOOL = 'banner';
...
export function fitBanner(
  spec: SheetSpec, pieceWIn = BANNER_W_IN, pieceHIn = BANNER_H_IN,
): SheetFit {
  const f = packBestOrientation({ ...BANNER_DEFAULTS, ...spec }, pieceWIn, pieceHIn, 'piece');
