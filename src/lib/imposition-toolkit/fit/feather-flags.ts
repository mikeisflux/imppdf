/* Feather Flags — large format, ONE piece per sheet.
 *
 * Wide-format work is printed to the piece, not ganged on a press sheet: the
 * media IS the feather flag. So there is no grid to compute and NO CUT MARKS —
 ...
 *
 * Rotation is off: a feather flag has a fixed orientation. Turning it to "fit
...
export const FEATHER_FLAG_W_IN = 30;
export const FEATHER_FLAG_H_IN = 100;
export const FEATHER_FLAG_TOOL = 'featherflag';

/** Large format never gangs and never marks. */
export const FEATHER_FLAG_DEFAULTS = {
  marginIn: 0, gutterXIn: 0, gutterYIn: 0,
  addMarks: false, autoRotate: false, buttCut: false,
} as const;

export function fitFeatherFlag(
  spec: SheetSpec, pieceWIn = FEATHER_FLAG_W_IN, pieceHIn = FEATHER_FLAG_H_IN,
): SheetFit {
  const f = packBestOrientation({ ...FEATHER_FLAG_DEFAULTS, ...spec }, pieceWIn, pieceHIn, 'piece');
