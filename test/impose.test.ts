...
test('cleanSubjectMask: fills pinholes, drops specks, keeps real gaps', async () => {
  const { cleanSubjectMask } = await import('../src/lib/imposition-toolkit/impose.ts');
  const w = 100, h = 100;
  const m = new Uint8Array(w * h);
  // A solid 60×60 body at (20,20).
  for (let y = 20; y < 80; y++) for (let x = 20; x < 80; x++) m[y * w + x] = 1;
  // A torn pinhole inside it, a speck out in the background, and a big gap
  // eaten out of one side that reaches the body's edge (an arm/body gap).
  for (let y = 40; y < 46; y++) for (let x = 40; x < 46; x++) m[y * w + x] = 0;
  m[5 * w + 5] = 1;
  for (let y = 60; y < 78; y++) for (let x = 62; x < 80; x++) m[y * w + x] = 0;

  const out = cleanSubjectMask(m, w, h, { closeRadius: 2, minIslandFrac: 0.05, maxHoleFrac: 0.2 });
  assert.equal(out[42 * w + 42], 255, 'the enclosed pinhole is filled');
  assert.equal(out[5 * w + 5], 0, 'the background speck is dropped');
  assert.equal(out[70 * w + 70], 0, 'a gap open to the background is left alone');
  assert.equal(out[25 * w + 25], 255, 'the body itself survives');
});

test('cleanSubjectMask: a subject running off the page keeps its edge', async () => {
  const { cleanSubjectMask } = await import('../src/lib/imposition-toolkit/impose.ts');
  // Full-bleed art always has the figure touching the frame; the close must
  // not treat off-page as empty and erode a gap along the border.
  const w = 40, h = 40;
  const m = new Uint8Array(w * h).fill(0);
  for (let y = 10; y < h; y++) for (let x = 0; x < 30; x++) m[y * w + x] = 1;
  const out = cleanSubjectMask(m, w, h, { closeRadius: 2 });
  assert.equal(out[(h - 1) * w + 0], 255, 'bottom-left corner stays in the subject');
